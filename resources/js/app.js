...
const app = createApp({
  ...
  methods: {
    ...
    addToCartFromDetail() {
      ...
      const quantity = this.selectedProduct.selectedQuantity || 1;
  ...
  template: `
    <div class="min-h-screen bg-gray-900">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <div v-for="product in filteredProducts" :key="product.id"
            ...
            <div class="p-4">
              ...
              <h3 class="font-bold text-lg mb-2 text-white cursor-pointer hover:text-purple-400 transition"
                  @click="viewProductDetail(product.id)">
                {{ product.name }}
              </h3>
              ...
              <div class="space-y-2">
                <button @click="viewProductDetail(product.id)"
      ...
      <div v-if="showProductDetail" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
        <div class="bg-gray-800 rounded-lg shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-auto border border-gray-700">
          <div class="flex justify-between items-center p-6 border-b border-gray-700">
            ...
            <button @click="closeProductDetail" class="text-gray-400 hover:text-white text-2xl">&times;</button>
          </div>

          <div v-if="productDetailLoading" class="p-12 text-center">
            <div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-purple-500"></div>
            <p class="mt-4 text-gray-400">Loading product details...</p>
          </div>

          <div v-else-if="selectedProduct" class="p-6">
            <div class="grid md:grid-cols-2 gap-8">
              <!-- Product Image -->
              <div>
                <img :src="selectedProduct.image || 'https://via.placeholder.com/600x400'"
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <h3 class="text-3xl font-bold text-white mb-2">{{ selectedProduct.name }}</h3>
                  <p class="text-gray-300 text-base leading-relaxed">{{ selectedProduct.description }}</p>
                </div>
                ...
                <div class="border-t border-gray-700 pt-4">
                  <div class="flex justify-between items-center mb-4">
                    ...
                    <div class="text-right">
                      ...
                      <span :class="selectedProduct.stock > 0 ? 'text-green-400' : 'text-red-400'"
                            class="font-semibold">
                  ...
                  <div v-if="selectedProduct.stock > 0" class="space-y-4">
                    <div>
                      <label class="block text-sm font-medium mb-2 text-gray-300">Quantity:</label>
                      <div class="flex items-center gap-4">
                        <button @click="selectedProduct.selectedQuantity = Math.max(1, selectedProduct.selectedQuantity - 1)"
                                class="w-12 h-12 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition text-xl font-bold">
                          -
                        </button>
                        <input v-model.number="selectedProduct.selectedQuantity"
                               type="number"
                               min="1"
                        ...
                        <button @click="selectedProduct.selectedQuantity = Math.min(selectedProduct.stock, selectedProduct.selectedQuantity + 1)"
                                class="w-12 h-12 bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition text-xl font-bold">
                          +
                        </button>
                      </div>
                ...
                <div class="border-t border-gray-700 pt-4">
                  ...
                  <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                      ...
                      <span class="text-white font-medium">#{{ selectedProduct.id }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-gray-400">Category:</span>
                      <span class="text-white font-medium">{{ selectedProduct.category || 'N/A' }}</span>
                    </div>
                    <div class="flex justify-between">
                      <span class="text-gray-400">Status:</span>
                      <span class="text-white font-medium">{{ selectedProduct.is_active ? 'Active' : 'Inactive' }}</span>
                    </div>
                  </div>
                </div>
              </div>
